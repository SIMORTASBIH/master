/*
  ...
  1. Remove Dependencies
    ...
    - Drop the has_role function safely
    - Recreate function with proper parameters

  2. Security Updates
    - Create permissive policies for all tables
    - Grant necessary permissions to authenticated users
    - Ensure all users have admin role for testing

  3. Testing
    - Create test function to verify database access
...
-- Step 2: Now we can safely drop the has_role function
DROP FUNCTION IF EXISTS has_role(uuid, app_role);

-- Step 3: Recreate the has_role function with proper parameter names
...
-- Conditional policies for tables that might exist
...
-- Step 5: Update the assign_default_role function
...
-- Step 8: Ensure all existing users have admin role
DO $$
DECLARE
...
-- Step 9: Create a test function to verify database access
...
-- Step 10: Create a simple function to check if RLS is working
